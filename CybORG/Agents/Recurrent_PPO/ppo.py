...
class PPO:
    def __init__(self, state_dimension, action_dimension, total_episodes, number=1):
        ...
        self.actor = Actor(state_dimension,action_dimension, init_log_std_dev=0.0)
        self.critic = Critic(state_dimension)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=self.lr)
    
    def get_action(self, state, terminal=None):
        """
        Args:
    ...
    def save_lstm_state(self):
        ...
        self.episodic_hidden_cells.append(self.actor.hidden_cell[1].squeeze(0).detach())
        self.episodic_critic_hidden.append(self.critic.hidden_cell[0].squeeze(0).detach())
        self.episodic_critic_cells.append(self.critic.hidden_cell[1].squeeze(0).detach())
    ...
    def evaluate(self, observations, actions):
        ...
        state_value = self.critic(observations).squeeze()
    ...
    def learn(self,total_steps):
        ...
        for i in range(self.epochs):
            ...
            for i in range(counter):
                # Initialize new hidden cells (based on the previously saved cells)
                self.actor.hidden_cell = (actor_hidden_states[i].unsqueeze(0), actor_cell_states[i].unsqueeze(0))
                self.critic.hidden_cell = (critic_hidden_states[i].unsqueeze(0), critic_cell_states[i].unsqueeze(0))
                # Get the data of the episode
                mini_obs = obs[i]
                mini_acts = acts[i]
                mini_log_prob = logprob[i]
                mini_log_prob = logprob[i]
                mini_advantage = A_k[i]
                mini_rtgs = rtgs[i]
                # Calculation of entropy, state values and action log probabilities for the episode
                mini_state_values, curr_log_probs, entropy = self.evaluate(mini_obs, mini_acts)
                # Step 5. Policy Loss
