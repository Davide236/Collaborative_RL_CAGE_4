...
def main():
    ...
    agent = PPO(env.observation_space.shape[0], env.action_space.n,50000, 1)
    rewards = []
    avg_rwd = []
    learning_steps = 0
    count = 0
    ...
    while count < 1000000:
        ...
        # TODO: Check if during the evaluation I do not need to reset the hidden state (basically dont resave them) - Delete PAdded memory when calculating loss
        print(f"EP REW: {total_reward} - AVG REW: {numpy.array(rewards).mean()}")
        if learn_goal >= 500:
            ...
            avg_rwd.append(numpy.array(rewards).mean())
            learning_steps += 1
            agent.learn(count) 
            learn_goal = 0
